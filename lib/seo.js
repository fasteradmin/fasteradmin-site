 ...
 * app/learn/[slug]/page.jsx and app/sitemap.js each had their own copy. Nothing
 ...
 * @param {string} [o.locale]      "nl_NL" or "en_US". Defaults to "en_US" so
 *                                 every existing call site is unaffected.
 * @param {object} [o.languages]   hreflang alternates, e.g. { nl: "/", en: "/eng" }.
 *                                 Only the homepage's two variants set this —
 *                                 a page with one language has nothing to
 *                                 alternate with.
 */
export function pageMetadata({
  ...
  const meta = {
    title,
    description,
    // Only ever set from a publishing payload's `keywords` array, never
    // hand-curated per page. Google ignores this tag; some answer engines and
    // internal crawlers still read it, and it costs nothing when it comes free
    // with the content.
      ...
      ...(languages
        ? {
            languages: Object.fromEntries(
              Object.entries(languages).map(([hreflang, p]) => [hreflang, absoluteUrl(p)])
            ),
          }
        : {}),
    },
...
/**
 * Organization + WebSite JSON-LD, rendered once per root layout (there are
 * now two: the Dutch root at `/` and the English root covering `/eng` and the
 ...
 *
 * The Organization node (name, logo, contact point) does not change with
 * locale — it is one legal entity either way. Only the WebSite node's
 * `inLanguage` and `description` should follow the page it is rendered on,
 * which is why this is a function now rather than a constant.
 *
 * @param {"nl"|"en"} [locale] Defaults to "en" so any caller that does not
 *                             pass one keeps today's behaviour.
 */
export const ORG_ID = `${SITE_URL}/#organization`;
export const SITE_ID = `${SITE_URL}/#website`;

const ORG_DESCRIPTION = {
  en: "FasterAdmin builds the operations systems that let a 20 to 50 person company handle more work without hiring for it.",
  nl: "FasterAdmin bouwt de operationele systemen waarmee een bedrijf van 20 tot 50 medewerkers meer werk aankan zonder ervoor aan te nemen.",
};

export function siteSchema(locale = "en") {
  return {
    ...
    "@graph": [
      {
        ...
        description: ORG_DESCRIPTION[locale] ?? ORG_DESCRIPTION.en,
